      ...
      try {
        ...
        if (param) {
          selectedAttr = attrs.find(
            a =>
              a.attribute_display_type === 'list' && a.attribute_key === param
          );
          console.log(
            '[DEBUG] Atributo selecionado via parâmetro:',
            selectedAttr?.attribute_key
          );
        }
        ...
        if (!selectedAttr) {
          selectedAttr = attrs.find(
            a =>
              a.attribute_display_type === 'list' &&
              a.attribute_key.startsWith('kbw_')
          );
          console.log(
            '[DEBUG] Atributo selecionado por fallback (kbw_):',
            selectedAttr?.attribute_key
          );
        }
        ...
        if (!selectedAttr) {
          ...
          console.log(
            '[DEBUG] Atributo selecionado por fallback (qualquer lista):',
            selectedAttr?.attribute_key
          );
        }
        ...
        if (lastAttributeKeyRef.current !== selectedAttr.attribute_key) {
          ...
          console.log(
            '[DEBUG] Atributo igual ao anterior, estado attribute não atualizado. Apenas contatos atualizados.'
          );
        }
      } catch (err) {
        setError(err);
...
export function useKanbanData() {
  ...
  const {
    ...
    loading,
    error,
  } = useDynamicKanbanData(reloadFlag);
  ...
  const { updateContactAttribute, updating, updateError } =
    useUpdateContactAttribute(reloadKanban, attribute, contacts);
