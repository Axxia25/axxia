...
function KanbanBoard() {
  ...
  useEffect(() => {
    if (!loading && contacts.length && columns.length) {
      ...
      contacts.forEach(contact => {
        const value =
          contact.custom_attributes?.[attribute.attribute_key] ??
          'Não definido';
  ...
  const onDragEnd = async ({ source, destination }) => {
    ...
    try {
      ...
      setNotification({
        type: 'error',
        message: `Erro ao atualizar estágio: ${err?.message || err}`,
      });
  ...
  if (error)
    return (
      <ErrorMessage message={`Erro ao carregar dados: ${error.message}`} />
    );
  if (!attribute)
    return (
      <ErrorMessage message="Nenhum atributo customizado do tipo lista encontrado." />
    );
